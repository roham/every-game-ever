...
const NAMES: Record<string, { title: string; desc: string; fmt: (v: number) => string }> = {
  ...
  clutch_swing: { title: "Clutchest 4th quarters", desc: "Largest 4th-quarter margin swing.", fmt: (v) => `${Math.round(v)}-point swing` },
  comeback_4q: { title: "4th-quarter comebacks", desc: "Trailed by 15+ entering the 4th, won.", fmt: (v) => `trailed by ${Math.round(v)}` },
};

export async function loadBoards(): Promise<Record<string, PrecedentRow[]>> {
  const rows = await q<PrecedentRow>(`SELECT * FROM '${"data/precedents.parquet"}'`);
...
export function renderPrecedents(
  ...
  for (const [key, rows] of Object.entries(boards)) {
    ...
    for (const r of rows.slice(0, 10)) {
      ...
      tr.innerHTML = `<td>${r.rank}</td><td>${r.game_date}</td><td>${r.home_team_id} vs ${r.away_team_id}</td>
        <td>${r.home_score}-${r.away_score}</td><td>${meta.fmt(r.value)}</td>`;
